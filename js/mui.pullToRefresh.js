			...
			if ($.isFunction(self.options.up.callback)) {
				self.pullUpTips = (function() {
					...
					if (!element) {
						...
						element.innerHTML = '<div class="mui-spinner"><div class="mui-loading"></div></div><span class="mui-pull-loading">' + self.options.up.contentrefresh + '</span>';
						self.element.appendChild(element);
					}
					self.pullUpTipsIcon = element.querySelector(SELECTOR_PULL_LOADING);
					self.pullUpspinner = element.querySelector('.mui-spinner');
					self.loadingClass = element.querySelector('.mui-loading');
					self.BOTTOM_TIPS = element;
					return element;
				}());
			}
		},
		...
		hiddenUpTis:function(finished){
			...
			if (finished) {
				...
				this.pullUpTipsIcon.innerHTML = this.options.up.contentnomore;
				  ...
				  this.BOTTOM_TIPS.classList.add(CLASS_HIDDEN);
			}
			this.loading = false;
		},
